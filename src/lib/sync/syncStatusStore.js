...
// survive /tarefas unmounting (tasksSyncScheduler.js keeps running after the
// user navigates away) and be readable from Topbar.jsx's subtree regardless
// of the current route, so a Context tied to some page's mount lifecycle
// wouldn't fit. Shaped for React's useSyncExternalStore (built into React
// 19, no new dependency) — see SyncStatusIndicator.jsx.
//
...
function defaultSnapshot() {
  return { tasks: defaultDomain(), settings: defaultDomain(), workspaces: defaultDomain() };
}
...
export function setTasksSyncState(patch) {
  setDomain('tasks', patch);
}
...
export function setWorkspacesSyncState(patch) {
  setDomain('workspaces', patch);
}
