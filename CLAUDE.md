...
### Questões (AI question generation via OpenAI)

`src/app/(dashboard)/questoes/page.jsx` + `src/components/QuestionGenerator.jsx` let a professor supply an OpenAI API key, list per-question specs (tema/nível/tipo — RU/CM/AR only, `INT` is never offered here), and get back a `quiz.schema.json`-shaped JSON they can preview and download.

- `src/lib/aiProviders/openai.js` holds a `SYSTEM_PROMPT` **condensed from the globally-installed `enade-it-questions` Claude Code skill** (`~/.claude/skills/enade-it-questions/`, not a repo file — the running app can't invoke the Skill tool at request time, so the rules are hardcoded as a template string). If that skill's rules change, `SYSTEM_PROMPT` needs manual updating to match.
- OpenAI's Structured Outputs strict mode (`text.format.type: 'json_schema'`, `strict: true`) doesn't support `contains`/`minContains`/`maxContains` — the exact mechanism `quiz.schema.json` uses (`$defs.question.properties.answers.allOf`) to enforce "exactly one correct answer". `openai.js` sends a derived copy of the schema with that `allOf` deleted and `course_id`/`quiz_id` narrowed from `["integer","null"]` to plain `"integer"` (we always emit `0`, never `null`). The "exactly one correct answer" rule becomes prompt guidance instead of an API-enforced constraint, and is re-checked after generation via the existing `validateStructural()` in `src/app/api/ai/openai/generate-questions/route.js` — same defense-in-depth pattern the Canvas import route already uses. There's no auto-retry: if it fails, the user just clicks "Gerar" again.
- The API key lives in `session.aiApiKeys = { openai: '...' }` (a keyed object, not a flat field) — this costs nothing extra now and avoids a session-shape rename when Claude/Gemini providers are added later. The routes themselves stay hardcoded under `/api/ai/openai/**`; a `[provider]` dynamic segment would be premature for a UI that only ever shows one provider today. No change was needed in `src/lib/session.js` itself — iron-session doesn't enforce a fixed shape, so `aiApiKeys` is just set directly by the route handlers, same as `accessToken`/`refreshToken` are today.
...
- `src/proxy.js`'s matcher includes `/questoes/:path*` and `/api/ai/:path*` alongside the Canvas-gated routes — `/questoes` needs the same login gate as `/courses`, and `/api/ai/*` is gated too so an unauthenticated caller can't spend a stored OpenAI key, even though it doesn't touch Canvas or need token refresh to function.
...
src/
  app/
    (dashboard)/  layout.jsx (Sidebar+Topbar shell) + page.jsx (capa/home), courses/**, questoes/ — route group, invisible in the URL, proxy-gated
    login/, api/**   outside the dashboard group, unauthenticated-reachable — no shell chrome
  components/     Sidebar.jsx, Topbar.jsx, CourseBrowser.jsx, StatusIcon.jsx, WebTechFooter.jsx + ImportQuestions.jsx, QuestionGenerator.jsx (client)
